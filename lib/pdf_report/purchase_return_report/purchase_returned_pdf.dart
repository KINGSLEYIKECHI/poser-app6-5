...
Future<void> generatePurchaseReturnReportPdf(
  ...
  if (data != null) {
    for (var item in data) {
      ...
      for (var purchaseReturn in item.purchaseReturns ?? []) {
        for (var detail in purchaseReturn.purchaseReturnDetails ?? []) {
  ...
  pw.Widget buildLocalizedTable(List<PurchaseTransaction> data) {
    final headers = [
      _lang.reference,
      _lang.date,
      _lang.supplier,
    ...
    final rows = data.map((item) {
      ...
      return [
        ...
        DateFormat('dd-MM-yyyy').format(DateTime.parse(item.purchaseDate.toString())),
        item.party?.name ?? 'n/a',
    ...
    // Reverse table for RTL (Arabic, Persian, Urdu, etc.)
    final displayedHeaders = isRTL ? headers.reversed.toList() : headers;
    final displayedRows = isRTL ? rows.map((r) => r.reversed.toList()).toList() : rows;

    // Create column widths for RTL
    final columnWidthsList = [
    ...
    final columnWidths = RtlTableHelper.createColumnWidths(widths: columnWidthsList, isRTL: isRTL);
        ...
        ...displayedRows.map((row) {
          return pw.TableRow(
            decoration: pw.BoxDecoration(
              color: displayedRows.indexOf(row).isOdd ? PdfColor.fromInt(0xffF7F7F7) : PdfColors.white,
            ),
            children: RtlTableHelper.reverseChildren(
                row.map((cell) {
                  return pw.Padding(
  ...
  try {
        ...
        header: (pw.Context context) => pw.Center(
          child: pw.Column(
            ...
            children: [
              ...
              localizedText(_lang.purchaseReturnReport, size: 16, bold: true),
    ...
    final file = File('${dir.path}/${appsName}-purchase-return-report.pdf');
