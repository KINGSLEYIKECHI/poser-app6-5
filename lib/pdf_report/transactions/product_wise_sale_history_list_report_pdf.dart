...
Future<void> generateProductWiseSaleHistoryReportPdf(
  ...
  pw.Widget buildLocalizedTable(phlm.ProductHistoryListModel data) {
    final headers = [
      _lang.sl,
      _lang.name,
      _lang.salesPrice,
    ...
    final rows = data.items?.map((item) {
          ...
          return [
            '$index',
            item.name ?? 'N/A',
            formatPointNumber(item.salePrice ?? 0, addComma: true),
            formatPointNumber(item.purchaseQuantity ?? 0),
            formatPointNumber(item.saleQuantity ?? 0),
            formatPointNumber(item.remainingQuantity ?? 0),
          ];
        }).toList() ??
        [];
    ...
    return RtlTableHelper.createTable(
      ...
      children: [
        ...
        // Data rows
        ...rows.map((row) {
          final isTotalRow = row[1] == _lang.total;
          return pw.TableRow(
  ...
  try {
    ...
    final file = File('${dir.path}/$appsName-product-sale-history.pdf');
