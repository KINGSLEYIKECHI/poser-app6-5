...
Future<void> generateBillWiseLossProfitReportPdf(
  ...
  pw.Widget localizedText(
    ...
    if (localeCode == 'bn') {
      return getLocalizedPdfText(
        ...
        pw.TextStyle(
          font: containsBangla(processedText) ? banglaFont : englishFont,
          fontSize: size,
          color: color,
  ...
  pw.Widget buildLocalizedTable() {
    ...
    for (int i = 0; i < (data.transactions?.length ?? 0); i++) {
      ...
      rows.add([
        '${i + 1}',
        t.invoiceNumber ?? '',
        t.transactionDate == null ? '' : intl.DateFormat('dd MMM, yyyy').format(t.transactionDate!),
        t.partyName ?? '',
        formatPointNumber(t.totalAmount ?? 0),
        formatPointNumber(t.isProfit ? (t.lossProfit ?? 0) : 0),
        formatPointNumber(t.isProfit ? 0 : (t.lossProfit ?? 0).abs()),
      ]);
    }
  ...
  try {
          ...
          child: pw.Column(
            children: [
              ...
              if (fromDate != null)
                localizedText(
                  '${_lang.duration}: '
                  '${intl.DateFormat('dd-MM-yyyy').format(fromDate)}'
                  ' - '
    ...
    final file = File('${dir.path}/$appsName-bill-wise-loss-profit.pdf');
