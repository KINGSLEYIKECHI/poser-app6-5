...
class TaxSaleInvoicePdf {
  static Future<void> generateSaleDocument(
    ...
    // FIXED: Local implementation to replace broken PDFCommonFunctions().getTaxProductQuantity
    // The external function was accessing 'salesReturnDetails' but the model uses 'details'
    num getTaxProductQuantity({required num detailsId, required TaxSaleData transactions}) {
      num originalQty = 0;
      // Find original quantity from sale details
      var saleDetail = transactions.sale?.salesDetails?.where((element) => element.id == detailsId).firstOrNull;
      if (saleDetail != null) {
        originalQty = saleDetail.quantities ?? 0;
      }

      num returnedQty = 0;
      // Subtract returned quantity from sales returns
      if (transactions.salesReturns?.isNotEmpty ?? false) {
        for (var returns in transactions.salesReturns!) {
          if (returns.details?.isNotEmpty ?? false) {
            for (var detail in returns.details!) {
              if (detail.saleDetailId == detailsId) {
                returnedQty += detail.returnQty ?? 0;
              }
            }
          }
        }
    ...
    /// Returns null if text is empty, preventing layout shifts
    pw.Widget? _optionalLocalizedText(String? text, {bool isRTL = false}) {
        ...
        children: [
          pw.Padding(
            ...
            child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
              ...
              if (detail.warrantyInfo?.warrantyDuration != null && detail.warrantyInfo?.warrantyUnit != null)
                localizedText(
                  '${_lang.warranty}: ${detail.warrantyInfo!.warrantyDuration} ${detail.warrantyInfo!.warrantyUnit}',
                ),

              if (detail.warrantyInfo?.guaranteeDuration != null && detail.warrantyInfo?.guaranteeUnit != null)
                localizedText(
                  '${_lang.guarantee}: ${detail.warrantyInfo!.guaranteeDuration} ${detail.warrantyInfo!.guaranteeUnit}',
                ),
            ]),
          )
        ],
    ...
    //               detail.saleDetail?.stock?.batchNo != null && detail.saleDetail!.stock!.batchNo!.isNotEmpty
    //                   ? localizedText(
    //                       '${_lang.batch}: ${detail.saleDetail!.stock!.batchNo}',
    //                       alignment: RtlTableHelper.adjustAlignment(pw.TextAlign.start, isRTL),
    //                     )
    //                   : localizedText(''),
    //               // Serial Numbers
        ...
        children: [
          for (var detail in details ?? [])
            pw.Padding(
              ...
              child: pw.Column(
                crossAxisAlignment: isRTL ? pw.CrossAxisAlignment.end : pw.CrossAxisAlignment.start,
                mainAxisSize: pw.MainAxisSize.min, // Prevents expansion issues
                children: [
                  // Product Name (always shown)
                  localizedText(
                    detail.saleDetail?.product?.productName ?? 'N/A',
                    alignment: RtlTableHelper.adjustAlignment(pw.TextAlign.start, isRTL),
                  ),

                  // Batch Number (only add widget if has value)
                  if (detail.saleDetail?.stock?.batchNo?.isNotEmpty == true)
                    localizedText(
                      '${_lang.batch}: ${detail.saleDetail!.stock!.batchNo}',
                      alignment: RtlTableHelper.adjustAlignment(pw.TextAlign.start, isRTL),
                    ),

                  // Serial Numbers (only add widget if has values)
                  if (detail.serialNumbers?.isNotEmpty == true)
                    localizedText(
                      '${_lang.serial}: ${detail.serialNumbers!.join(", ")}',
                      alignment: RtlTableHelper.adjustAlignment(pw.TextAlign.start, isRTL),
                    ),
                    ...
                    children: [
                      ...
                      pw.Expanded(
                        child: pw.Table(
                          ...
                          children: [
                            _buildTableRow(_lang.invoiceNo, transactions.sale?.invoiceNumber ?? 'n/a',
                                _lang.invoiceDate, transactions.sale?.saleDate ?? 'n/a'),
                            _buildTableRow(_lang.orderReference, transactions.sale?.invoiceNumber ?? '',
                                _lang.paymentReference, transactions.sale?.paymentType?.name ?? 'N/A'),
                          ],
                        ),
                      ),
                    ],
                        ...
                        pw.Padding(
                          ...
                          child: pw.Column(
                            ...
                            children: [
                              ...
                              localizedText('${_lang.state}: ${transactions.sale?.business?.state?.name ?? 'n/a'}'),
                              localizedText('${_lang.email}: ${transactions.sale?.business?.email ?? 'n/a'}',
                                  bold: true),
                            ],
                          ),
                        ),
                        ...
                        pw.Padding(
                          ...
                          child: pw.Column(
                            ...
                            children: [
                              ...
                              localizedText('${_lang.phone}: ${transactions.sale?.party?.phone ?? 'n/a'}'),
                              localizedText('${_lang.email}: ${transactions.sale?.party?.email ?? 'n/a'}', bold: true),
                            ],
                          ),
                        ),
                      ],
                        ...
                        child: pw.Column(children: [
                          pw.Column(
                            ...
                            children: [
                              ...
                              // ),
                              localizedText(
                                "${transactions.sale?.vat?.name ?? (transactions.taxItems?.isNotEmpty ?? false ? _lang.vat : _lang.vat)}: ${formatPointNumber(vatAmount)}",
                                bold: true,
                              ),
                              pw.SizedBox(height: 5.0),
                              localizedText(
                                  ...
                                  children: [
                                    ...
                                    ...(transactions.taxItems?.isNotEmpty ?? false
                                        ? transactions.taxItems!.map((e) {
                                            return _taxCell('${e.vatRate}%', '${e.vatAmount}');
                                          }).toList()
                                        : []),
                                    _bodyCell(
